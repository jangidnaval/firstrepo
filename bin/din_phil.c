...
int main()
{
     ...
     int i;
     for(i=0; i<5; i++)
          sh.mutex[i] = sem_open(name[i], O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP| S_IWGRP, 1);//creating a named semaphore with given attributes
     ...
     for(i=0; i<5; i++)
          sem_unlink(name[i]); //closing a named semaphore
